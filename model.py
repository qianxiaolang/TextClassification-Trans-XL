...
Q=tf.layers.dense(ffc_output,units=80,kernel_initializer=tf.random_normal_initializer(stddev=1))
q=tf.nn.softmax(tf.reshape(Q,[-1,10,8]),axis=-1)


K=tf.layers.dense(ffc_output,units=80,kernel_initializer=tf.random_normal_initializer(stddev=1))
k=tf.reshape(K,[-1,10,8])
